...
/**
 * scrabble.Scrabble board, maintaining bag, players' hands, and other game logic.
 *
 * General conventions:
 *
...
public class Board {
    ...
    /** Width of this scrabble.Board, in squares. */
    ...
    private void deal(List<Character> hand, int n) {
        for (int i = 0; i < n; i++) {
            if (bag.isEmpty()) {
    ...
    /** Throws an scrabble.IllegalMoveException if playing word at location in direction from hand would not be legal. */
    public void verifyLegality(String word, Location location, Location direction, List<Character> hand) throws IllegalMoveException {
        ...
        if (!canBePlacedOnBoard(word, location, direction)) {
            throw new IllegalMoveException("scrabble.Board placement incorrect (gaps, overlapping tiles, edge of board).");
        }
